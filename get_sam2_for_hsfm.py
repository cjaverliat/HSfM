...
def main(
    ...
    try:
        ...
        print(
            "Only JPEG frames are supported by SAM2, whose file names should be like <frame_index>.jpg"
        )
        print(
            "Copying the frames to a new directory and renaming them to <frame_index>.jpg..."
        )
        if img_dir[-1] == "/":
            img_dir = img_dir[:-1]
        new_img_dir = os.path.join(
            os.path.dirname(img_dir), f"new_{os.path.basename(img_dir)}"
        )
        ...
        for _, fn in tqdm(enumerate(frame_names)):
            ...
            shutil.copy(
                os.path.join(img_dir, fn), os.path.join(new_img_dir, new_frame_name)
            )
    ...
    if vis:
        ...
        CommonUtils.draw_masks_and_box_with_supervision(
            img_dir, mask_data_dir, json_data_dir, result_dir
        )
