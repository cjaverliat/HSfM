...
coco_main_body_joint_names = COCO_WHOLEBODY_KEYPOINTS[
    : coco_main_body_end_joint_idx + 1
]
smplx_main_body_joint_idx = [
    ORIGINAL_SMPLX_JOINT_NAMES.index(joint_name)
    for joint_name in coco_main_body_joint_names
]
...
def torch_angle_axis_to_rotation_matrix(angle_axis):
    ...
    rotation_matrix = (
        torch.eye(3, device=angle_axis.device)
        + torch.sin(angle) * K
...
def parse_to_save_data(scene, cam_names, main_cam_idx=None):
    ...
    if main_cam_idx is not None:
        ...
        for i, cam_name in enumerate(cam_names):
            ...
            pts3d[i] = (
                pts3d[i] @ main_cam_world2cam[:3, :3].T + main_cam_world2cam[:3, 3:].T
            )
...
def init_human_params(
    smplx_layer,
    body_model_name,
    multiview_multiple_human_cam_pred,
    multiview_multiperson_pose2d,
    focal_length,
    princpt,
    device="cuda",
    get_vertices=False,
    ...
    first_cam_human_name_counts = {
        human_name: {"count": 0, "pose2d_conf": 0}
        for human_name in sorted(
            list(multiview_multiple_human_cam_pred[first_cam].keys())
        )
    }
    missing_human_names_in_first_cam = defaultdict(list)
    multiview_multiperson_init_trans = defaultdict(
        dict
    )  # Dict[human_name -> Dict[cam_name -> (3)]]
    for cam_name in camera_names:
        for human_name in sorted(
            list(multiview_multiple_human_cam_pred[cam_name].keys())
        ):
            ...
            if body_model_name == "smplx":
                ...
                left_hand_pose = (
                    params["left_hand_pose"].reshape(1, -1).to(device)
                    if "left_hand_pose" in params
                    else None
                )
                right_hand_pose = (
                    params["right_hand_pose"].reshape(1, -1).to(device)
            ...
            if body_model_name == "smplx":
                smplx_coco_main_body_joints = smplx_joints[
                    0, smplx_main_body_joint_idx, :
                ]  # .to(device)
            elif body_model_name == "smpl":
                smplx_coco_main_body_joints = smplx_joints[
                    0, smpl_main_body_joint_idx, :
                ]  # .to(device)
            vitpose_2d_keypoints = multiview_multiperson_pose2d[human_name][cam_name][
                coco_main_body_joint_idx
            ]  # .cpu().numpy() # (J, 2+1)
            init_trans = estimate_initial_trans(
                smplx_coco_main_body_joints,
                vitpose_2d_keypoints,
                focal_length,
                princpt,
                COCO_MAIN_BODY_SKELETON,
            )
            ...
            if human_name in sorted(list(first_cam_human_name_counts.keys())):
                ...
                first_cam_human_name_counts[human_name]["pose2d_conf"] = sum(
                    vitpose_2d_keypoints[:, 2]
                )
            else:
    ...
    if max_count != len(camera_names):
        print(
            f"Warning: {main_human_name_candidates} are the most detected main human but not detected in all cameras"
        )
    ...
    global_orient_first_cam = multiview_multiple_human_cam_pred[first_cam][
        main_human_name
    ]["params"]["global_orient"][0]  # .to(device)
    # Convert axis angle to rotation matrix using torch
    if body_model_name == "smplx":
        global_orient_first_cam = torch_angle_axis_to_rotation_matrix(
            global_orient_first_cam
        ).reshape(3, 3)
    ...
    for cam_name in sorted(
        list(multiview_multiperson_init_trans[main_human_name].keys())
    ):
        if cam_name == first_cam:
            continue

        # Get human orientation and position in other camera
        global_orient_other_cam = multiview_multiple_human_cam_pred[cam_name][
            main_human_name
        ]["params"]["global_orient"][0]  # .to(device)
        if body_model_name == "smplx":
            global_orient_other_cam = torch_angle_axis_to_rotation_matrix(
                global_orient_other_cam
            ).reshape(3, 3)
        init_trans_other_cam = multiview_multiperson_init_trans[main_human_name][
            cam_name
        ]

        # Calculate rotation and translation
        R_other = torch.matmul(
            global_orient_first_cam, global_orient_other_cam.transpose(0, 1)
        )
    ...
    for human_name in sorted(list(multiview_multiple_human_cam_pred[first_cam].keys())):
        first_cam_human_params[human_name] = multiview_multiple_human_cam_pred[
            first_cam
        ][human_name]["params"]
        first_cam_human_params[human_name]["root_transl"] = (
            multiview_multiperson_init_trans[human_name][first_cam].reshape(1, -1)
        )  # .to(device)
    ...
    if True:
        ...
        try:
            for missing_human_name in missing_human_names_in_first_cam:
                ...
                other_cam_name = missing_human_names_in_first_cam[missing_human_name][
                    missing_human_exist_cam_idx
                ]
                ...
                missing_human_params_in_other_cam = multiview_multiple_human_cam_pred[
                    other_cam_name
                ][missing_human_name]["params"]
                ...
                other_cam_to_first_cam_transformation = cam_poses[
                    other_cam_name
                ]  # (4,4)
                missing_human_params_in_other_cam_global_orient = (
                    missing_human_params_in_other_cam["global_orient"][0].cpu().numpy()
                )  # (3,)
                missing_human_params_in_other_cam_global_orient = (
                    R.from_rotvec(missing_human_params_in_other_cam_global_orient)
                    .as_matrix()
                    .astype(np.float32)
                )  # (3,3)
                missing_human_params_in_other_cam_global_orient = (
                    other_cam_to_first_cam_transformation[:3, :3]
                    @ missing_human_params_in_other_cam_global_orient
                )  # (3,3)
                missing_human_params_in_other_cam["global_orient"] = torch.from_numpy(
                    R.from_matrix(missing_human_params_in_other_cam_global_orient)
                    .as_rotvec()
                    .astype(np.float32)
                ).to(device)  # (3,)

                missing_human_init_trans_in_other_cam = (
                    multiview_multiperson_init_trans[missing_human_name][other_cam_name]
                )
                missing_human_init_trans_in_first_cam = (
                    other_cam_to_first_cam_transformation[:3, :3]
                    @ missing_human_init_trans_in_other_cam
                    + other_cam_to_first_cam_transformation[:3, 3]
                )
                # compenstate rotation (translation from origin to root joint was not cancled)
                root_transl_compensator = (
                    other_cam_to_first_cam_transformation[:3, :3]
                    @ missing_human_params_in_other_cam["org_cam_root_transl"]
                )
                missing_human_init_trans_in_first_cam = (
                    missing_human_init_trans_in_first_cam + root_transl_compensator
                )
                #
                missing_human_params_in_other_cam["root_transl"] = (
                    torch.from_numpy(missing_human_init_trans_in_first_cam)
                    .reshape(1, -1)
                    .to(device)
                )

                first_cam_human_params[missing_human_name] = (
                    missing_human_params_in_other_cam
                )
        except Exception:
            print(
                "Warning: Some humans are missing in the first camera view and cannot be handled"
            )
    ...
    if get_vertices:
        ...
        for human_name, human_params in first_cam_human_params.items():
            ...
            if body_model_name == "smplx":
                ...
                global_orient = global_orient.reshape(1, -1).to(device)
                left_hand_pose = (
                    human_params["left_hand_pose"].reshape(1, -1).to(device)
                    if "left_hand_pose" in human_params
                    else None
                )
                right_hand_pose = (
                    human_params["right_hand_pose"].reshape(1, -1).to(device)
                ...
                global_orient = human_params["global_orient"][
                    None, :, :
                ]  # (1, 1, 3, 3)
            ...
            vertices = (
                vertices
                - joints[0:1]
                + human_params["root_transl"].detach().cpu().numpy()
            )
    ...
    for human_name, human_params in first_cam_human_params.items():
        ...
        if body_model_name == "smpl":
            ...
            optim_target_dict[human_name]["global_orient"] = (
                nn.Parameter(global_orient_6d).float().to(device)
            )
            body_pose_6d = matrix_to_rotation_6d(human_params["body_pose"])
            optim_target_dict[human_name]["body_pose"] = (
                nn.Parameter(body_pose_6d).float().to(device)
            )
        else:
            optim_target_dict[human_name]["global_orient"] = nn.Parameter(
                human_params["global_orient"].float().to(device)
            )
            optim_target_dict[human_name]["body_pose"] = nn.Parameter(
                human_params["body_pose"].float().to(device)
            )

        optim_target_dict[human_name]["betas"] = nn.Parameter(
            human_params["betas"].float().to(device)
        )
        optim_target_dict[human_name]["root_transl"] = nn.Parameter(
            human_params["root_transl"].float().to(device)
        )  # (1, 3)

        if body_model_name == "smplx":
            for param_name in optim_target_dict[human_name].keys():
                # reshape to (1, -1)
                optim_target_dict[human_name][param_name] = nn.Parameter(
                    human_params[param_name].reshape(1, -1).float().to(device)
                )
            if "left_hand_pose" in human_params:
                optim_target_dict[human_name]["left_hand_pose"] = nn.Parameter(
                    human_params["left_hand_pose"].float().to(device)
                )  # (1, 45)
            else:
                optim_target_dict[human_name]["left_hand_pose"] = nn.Parameter(
                    torch.zeros(1, 45).float().to(device)
                )  # (1, 45)
            if "right_hand_pose" in human_params:
                optim_target_dict[human_name]["right_hand_pose"] = nn.Parameter(
                    human_params["right_hand_pose"].float().to(device)
                )  # (1, 45)
            else:
                optim_target_dict[human_name]["right_hand_pose"] = nn.Parameter(
                    torch.zeros(1, 45).float().to(device)
...
def get_human_loss(
    smplx_layer_dict,
    body_model_name,
    humans_optim_target_dict,
    cam_names,
    multiview_world2cam_4by4,
    multiview_intrinsics,
    multiview_multiperson_poses2d,
    multiview_multiperson_bboxes,
    only_main_body_joints=True,
    ...
    if body_model_name == "smplx":
        body_pose = torch.cat(
            [
                humans_optim_target_dict[name]["body_pose"].reshape(1, -1)
        ...
        betas = torch.cat(
            [
                humans_optim_target_dict[name]["betas"].reshape(1, -1)
                for name in human_names
            ],
            dim=0,
        )
        global_orient = torch.cat(
            [
                humans_optim_target_dict[name]["global_orient"].reshape(1, -1)
                for name in human_names
            ],
            dim=0,
        )
        root_transl = torch.cat(
            [
                humans_optim_target_dict[name]["root_transl"].reshape(1, 1, -1)
                for name in human_names
            ],
            dim=0,
        )
        left_hand_pose = torch.cat(
            [
                humans_optim_target_dict[name]["left_hand_pose"].reshape(1, -1)
                for name in human_names
            ],
            dim=0,
        )
        right_hand_pose = torch.cat(
            [
                humans_optim_target_dict[name]["right_hand_pose"].reshape(1, -1)
                for name in human_names
            ],
            dim=0,
        )
        ...
        body_pose = torch.cat(
            [
                humans_optim_target_dict[name]["body_pose"].reshape(1, 23, 6)
                for name in human_names
            ],
            dim=0,
        )
        betas = torch.cat(
            [
                humans_optim_target_dict[name]["betas"].reshape(1, 10)
                for name in human_names
            ],
            dim=0,
        )
        global_orient = torch.cat(
            [
                humans_optim_target_dict[name]["global_orient"].reshape(1, 1, 6)
                for name in human_names
            ],
            dim=0,
        )
        root_transl = torch.cat(
            [
                humans_optim_target_dict[name]["root_transl"].reshape(1, 1, 3)
                for name in human_names
            ],
            dim=0,
        )
    ...
    world2cam_expanded = multiview_world2cam_4by4.unsqueeze(0).expand(
        B, -1, -1, -1
    )  # (B, N, 4, 4)
    intrinsics_expanded = multiview_intrinsics.unsqueeze(0).expand(
        B, -1, -1, -1
    )  # (B, N, 3, 3)
    ...
    if num_of_humans_for_optimization == batch_size:
        points_homo = torch.cat(
            (smplx_j3d_expanded, torch.ones((B, N, J, 1), device=device)), dim=3
        )  # (B, N, J, 4)
        points_cam = torch.matmul(
            world2cam_expanded, points_homo.transpose(2, 3)
        )  # (B, N, 4, J)
        points_img = torch.matmul(
            intrinsics_expanded, points_cam[:, :, :3, :]
        )  # (B, N, 3, J)
        ...
        world2cam_expanded_detached = world2cam_expanded[
            num_of_humans_for_optimization:
        ].detach()
        intrinsics_expanded_detached = intrinsics_expanded[
            num_of_humans_for_optimization:
        ].detach()
        world2cam_expanded = torch.cat(
            (
                world2cam_expanded[:num_of_humans_for_optimization],
                world2cam_expanded_detached,
            ),
            dim=0,
        )
        intrinsics_expanded = torch.cat(
            (
                intrinsics_expanded[:num_of_humans_for_optimization],
                intrinsics_expanded_detached,
            ),
            dim=0,
        )
        points_cam = torch.matmul(
    ...
    for human_idx, human_name in enumerate(human_names):
        ...
        human_proj_joints_coco = torch.zeros(
            len(cam_indices),
            len(COCO_WHOLEBODY_KEYPOINTS),
            3,
            device=device,
            dtype=torch.float32,
        )
        for i, joint_name in enumerate(COCO_WHOLEBODY_KEYPOINTS):
            if body_model_name == "smplx" and joint_name in ORIGINAL_SMPLX_JOINT_NAMES:
                human_proj_joints_coco[:, i, :2] = human_proj_joints[
                    :, ORIGINAL_SMPLX_JOINT_NAMES.index(joint_name), :2
                ]
                human_proj_joints_coco[:, i, 2] = 1
            elif body_model_name == "smpl" and joint_name in SMPL_45_KEYPOINTS:
                human_proj_joints_coco[:, i, :2] = human_proj_joints[
                    :, SMPL_45_KEYPOINTS.index(joint_name), :2
                ]
                human_proj_joints_coco[:, i, 2] = 1

        # Weight main body joints more heavily
        human_proj_joints_coco[
            :, : COCO_WHOLEBODY_KEYPOINTS.index("right_heel") + 1, 2
        ] *= 10

        # Get only main body keypoints
        if only_main_body_joints:
            human_proj_joints_coco = human_proj_joints_coco[
                :, coco_main_body_joint_idx, :
            ]
        ...
        one_human_loss = (
            loss_weights[:, None, None].repeat(1, human_proj_joints_coco.shape[1], 1)
            * human_proj_joints_coco[:, :, 2:]
            * poses2d[:, :, 2:]
            * F.mse_loss(
                human_proj_joints_coco[:, :, :2], poses2d[:, :, :2], reduction="none"
        ...
        for idx, cam_idx in enumerate(cam_indices):
            projected_joints[cam_names[cam_idx]][human_name] = human_proj_joints_coco[
                idx
            ]
...
def convert_human_params_to_numpy(human_params):
    ...
    for human_name, optim_target_dict in human_params.items():
        ...
        for param_name in optim_target_dict.keys():
            human_params_np[human_name][param_name] = (
                optim_target_dict[param_name].reshape(1, -1).detach().cpu().numpy()
            )
...
def main(
    ...
    for frame_idx, frame_name in zip(frame_idx_list, frame_names):
        ...
        for person_id in person_ids:
            ...
            if body_model_name == "smpl":
                ...
                smplx_params_tensor = {
                    key: torch.tensor(
                        smplx_data[key], device=device, dtype=torch.float32
                    )
    ...
    for cam_name in sorted(list(multiview_multiple_human_cam_pred.keys())):
        for human_name in sorted(
            ...
            pose2d = torch.tensor(
                multiview_multiple_human_cam_pred[cam_name][human_name]["pose2d"],
                device=device,
                dtype=torch.float32,
            )
            bbox = torch.tensor(
                multiview_multiple_human_cam_pred[cam_name][human_name]["bbox"],
                device=device,
                dtype=torch.float32,
            )
            ...
            bbox = torch.cat(
                [
                    bbox[:4],
                    torch.tensor([bbox_conf], device=device, dtype=torch.float32),
                ],
                dim=0,
            )
            ...
            affine_transform = torch.tensor(
                multiview_affine_transforms[cam_name],
                device=device,
                dtype=torch.float32,
            )
            ...
            dust3r_img_h, dust3r_img_w = multiview_images[cam_name].shape[
                :2
            ]  # (512, 288)
            # sanity check; if there's any pose2d_transformed's xy is out of the image size, make such joints' confidence 0
            pose2d_transformed[:, 2] = (
                pose2d_transformed[:, 2]
                * (pose2d_transformed[:, 0] < dust3r_img_w)
                * (pose2d_transformed[:, 1] < dust3r_img_h)
            )
            pose2d_transformed[:, 2] = (
                pose2d_transformed[:, 2]
                * (pose2d_transformed[:, 0] > 0)
                * (pose2d_transformed[:, 1] > 0)
            )

            # Transform bbox
            bbox_reshaped = torch.tensor(
                [[bbox[0], bbox[1], 1], [bbox[2], bbox[3], 1]],
                device=device,
                dtype=torch.float32,
            )
    ...
    try:
        if len(human_inited_cam_locations) > 2:
            ...
            hmr2_cam_dist = torch.norm(
                human_inited_cam_locations[0] - human_inited_cam_locations[1]
            )
            dust3r_cam_dist = torch.norm(
                dust3r_cam_locations[0] - dust3r_cam_locations[1]
            )
            # check dust3r failure
            if dust3r_cam_dist < 1e-3:  # camera distance is too small
                raise ValueError(
                    f"Maybe Dust3r failure; Dust3r camera distance is too small: {dust3r_cam_dist:.3f}"
                )
    ...
    scene = global_aligner(
        dust3r_network_output,
        device=device,
        mode=mode,
        verbose=not silent,
        focal_break=focal_break,
    )
    ...
    root_transl_cam = (
        multiview_world2cam_4by4[:, None, :3, :3]
        @ root_transl_list[None, :, :, :].transpose(2, 3)
    ).transpose(2, 3) + multiview_world2cam_4by4[:, None, :3, 3:].transpose(
        2, 3
    )  # (len(cam_names), N, 1, 3)
    ...
    while (
        not (root_transl_cam[:, 2] > 0).all() or scene_scale < min_scene_scale
    ) and scale_update_iter < max_scale_update_iter:
        ...
        root_transl_cam = (
            multiview_world2cam_4by4[:, None, :3, :3]
            @ root_transl_list[None, :, :, :].transpose(2, 3)
        ).transpose(2, 3) + res_scale * multiview_world2cam_4by4[
            :, None, :3, 3:
        ].transpose(2, 3)  # (len(cam_names), N, 1, 3)
        root_transl_cam = root_transl_cam.reshape(-1, 3)  # (len(cam_names) * N, 3)
        scale_update_iter += 1
    ...
    stage2_iter = list(
        range(
            int(niter * stage2_start_idx_percentage),
            int(niter * stage3_start_idx_percentage),
        )
    )
    ...
    with tqdm.tqdm(total=niter) as bar:
        while bar.n < bar.total:
            ...
            if bar.n in stage1_iter:
                ...
                multiview_cam2world_3by4 = torch.cat(
                    [
                        ...
                        (
                            multiview_cam2world_4by4[:, :3, 3] * residual_scene_scale
                        ).unsqueeze(-1),
                    ],
                    dim=2,
                )
                multiview_cam2world_4by4 = torch.cat(
                    [multiview_cam2world_3by4, multiview_cam2world_4by4[:, 3:4, :]],
                    dim=1,
                )
            ...
            only_main_body_joints = bar.n not in stage3_iter
            losses["human_loss"], projected_joints = get_human_loss(
                ...
                multiview_multiperson_poses2d,
                multiview_multiperson_bboxes,
                only_main_body_joints=only_main_body_joints,
    ...
    if body_model_name == "smpl":
        for person_id in human_params.keys():
            human_params[person_id]["global_orient"] = rotation_6d_to_matrix(
                human_params[person_id]["global_orient"]
            )
