...
def visualize_cameras_and_human(
    cam_poses,
    human_vertices,
    smplx_faces,
    world_colmap_pointcloud_xyz=None,
    world_colmap_pointcloud_rgb=None,
):
...
def show_env_human_in_viser(
    world_env: dict = None,
    world_env_pkl: str = "",
    world_scale_factor: float = 1.0,
    smplx_vertices_dict: dict = None,
    smplx_faces: np.ndarray = None,
    gt_cameras: dict = None,
):
    ...
    def _(_) -> None:
        for pc in pointcloud_handles:
            pc.point_ball_norm = 2.0 if gui_point_shape.value == "circle" else np.inf
        ...
        while True:
            ...
            with server.atomic():
                ...
                for img_idx, img_name in enumerate(sorted(world_env.keys())):
                    ...
                    mask = (
                        world_env[img_name]["conf"].flatten()
                        >= gui_confidence_threshold.value
                    )

                    # show human pointcloud
                    if (
                        not gui_show_human_pointcloud.value
                        and "sam2_mask" in world_env[img_name].keys()
                    ):
                ...
                if len(human_mesh_handles) == 0 and smplx_vertices_dict is not None:
                    for img_idx, img_name in enumerate(
                        sorted(smplx_vertices_dict.keys())
                    ):
                ...
                if control_distance_measurement is None:
                    control0 = server.scene.add_transform_controls(
                        "/controls/0", position=camera_frustums[0].position, scale=0.5
                    )
                    control1 = server.scene.add_transform_controls(
                        "/controls/1", position=camera_frustums[1].position, scale=0.5
                    )
                    distance_text = server.gui.add_text(
                        "Distance", initial_value="Distance: 0"
                    )
...
def show_optimization_results(world_env, body_model_name, human_params, smplx_layer):
    ...
    for human_name, optim_target_dict in human_params.items():
        if body_model_name == "smplx":
            ...
            body_pose = optim_target_dict["body_pose"].reshape(
                1, 23, 3, 3
            )  # (1, 23, 3, 3)
            global_orient = optim_target_dict["global_orient"].reshape(
                1, 1, 3, 3
            )  # (1, 1, 3, 3)
            betas = optim_target_dict["betas"].reshape(1, 10)  # (1, 10)
            smplx_output = smplx_layer(
                body_pose=body_pose,
                betas=betas,
                global_orient=global_orient,
    ...
    try:
        show_env_human_in_viser(
            ...
            smplx_vertices_dict=smplx_vertices_dict,
            smplx_faces=smplx_layer.faces,
        )
    except Exception:
        import pdb
